...
async function saveRule() {
  ...
  if (!form.value.targetHost) {
    ElMessage.error('Please enter the backend address')
    return
  }
...
<template>
  <div class="reverse-proxy">
    <el-card>
      ...
      <el-empty v-if="rules.length === 0 && !store.loading" description="No reverse proxy rules">
    ...
    >
      <el-form :model="form" label-width="120px">
        ...
        <el-form-item label="Backend Host" required>
          <el-input v-model="form.targetHost" placeholder="IP or hostname, e.g., 127.0.0.1" />
        </el-form-item>
        <el-form-item label="Backend Port" required>
          <el-input-number
        ...
        <el-form-item label="Listen Port" required>
          ...
          />
          <div class="form-tip">The port to expose. Access via http://localhost:{{ form.localPort || 'PORT' }}</div>
        </el-form-item>
